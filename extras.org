...
**** BROKEN COMMENT updated version
fix this

#+begin_src emacs-lisp  :eval no
  (defun org-roam--create-basic-capture-template-v2 (name &optional key links)
    "Create a basic capture template with NAME and shortcut KEY (default: first
    ...
    (let ((key (or key
      ...
      (list
       (if links
         ...
         (progn key (capitalize name) 'plain "\n%?"))
       :if-new (list 'file+head (concat (file-name-as-directory name)
                                        slug)
                     default-title)
       :immediate-finish t
  ...
  ;; ELISP> (mapcar #'(lambda (cat) (list cat nil t)) '("foo" "bar" "baz"))
  ;; (("foo" nil t)
  ;;  ("bar" nil t)
  ;;  ("baz" nil t))
  ...
  ;; ("foo" :if-new
  ;;  (bar baz))
  ;; so progn isn't going to work like you thought

  ;; ELISP> (let ((boolean nil))
  ;;          (append
  ;;           (if boolean
  ;;               (list "q" 'foo)
  ;;             (list "z" 'bar))
  ;;           (list :if-new (list 'baz boolean 'quux))))
  ;; ("z" bar :if-new
  ;;  (baz nil quux))
  ...
  ;; ELISP> (org-roam--create-basic-capture-template-v2 "main" nil nil)
  ;; ("m" "Main" plain "
  ;; %?" :if-new
  ...
  ;; ELISP> (org-roam--create-basic-capture-template-v2 "main" nil t)
  ;; ("M" "Main (w/links section)" plain "
  ;; %?
  ;; * Org-roam links                           :noexport:
  ;; - " :if-new
  ;; (file+head "main/${slug}.org" "#+title: ${title}
  ...
  ;; ELISP> (append '(("foo")) org-roam-capture-templates)
  ;; (("foo")
  ;;  ("m" "main" plain "%?" :if-new
  ;;   (file+head "main/${slug}.org" "#+title: ${title}
  ;; ")
  ...
  ;; ")
  ;;   :immediate-finish t :unnarrowed t)
  ;;  ("s" "synthesis" plain "%?" :if-new
  ;;   (file+head "synthesis/${slug}.org" "#+title: ${title}
  ;; ")
  ;;   :immediate-finish t :unnarrowed t))


  ;; ELISP> (org-roam--create-basic-capture-template-v2 "main")
  ;; ("
  ;; %?" :if-new
  ;; (file+head "main/${slug}.org" "#+title: ${title}
  ;; ")
  ;; :immediate-finish t :unnarrowed t)
  ;; ELISP> (org-roam--create-basic-capture-template-v2 '("main" nil t))
  ;; *** Eval error ***  Wrong type argument: characterp, "main"
  ...
  ;; (let ((extended-categories '(("foo" nil t)
  ;;                              ("bar" nil t)
  ;;                              ("baz" "B" t))))
  ;;   (mapcar #'(lambda (ext-cat)
  ;;               (apply #'org-roam--create-basic-capture-template-v2
  ;;                      ext-cat))
  ;;           extended-categories))
  ...
  ;; (setq org-roam-capture-templates-v2
  ;;       (let ((categories '("main" "references" "synthesis")))
  ;;         ;; (append (mapcar #'org-roam--create-basic-capture-template-v2
  ;;         ;;                 categories)
  ;;                 (mapcar #'org-roam--create-basic-capture-template-v2
  ;;                         (mapcar #'(lambda (cat)
  ;;                                     (list cat nil t))
  ;;                                 categories))
  ;;                 ;; )
